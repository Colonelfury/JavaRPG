...
class Enemy(Character):
  ...
  def __init__(self, player):
    Character.__init__(self)  # Init the Character class too.
    self.name = choice(['a goblin', 'a giant spider', 'a skeleton', 'a giant snake'])
...
class Player(Character):
  ...
  def score(self):
    print 'You have kill %s creeps' % (self.creep_kills)
...
#  loops until the player's health goes to 0
while(p.health > 0):
  line = raw_input("> ")
  args = line.split()
  if len(args) > 0:
...
p.score()  # While loop broke out...ending the program